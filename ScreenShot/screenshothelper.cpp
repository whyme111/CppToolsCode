...
Qt::WindowState ScreenShotHelper::mdiWindowInitState = Qt::WindowNoState;
bool ScreenShotHelper::clickCloseToTray = true;
bool ScreenShotHelper::ignoreClickCloseToTray = false;
bool ScreenShotHelper::hotKeyNoWait = true;
bool ScreenShotHelper::includeCursor = false;
bool ScreenShotHelper::noBorder = false;
bool ScreenShotHelper::copyToClipBoardAfterSnap = false;
...
QPixmap ScreenShotHelper::grabWindow(int snapMethod, HWND hwnd, int type, bool includeCursor, int x, int y, int w , int h)
{
    ...
    //QImage image = QtWin::fromHBITMAP(bitmap);

    QImage image = (QtWin::fromHBITMAP(bitmap)).toImage();
...
QPixmap ScreenShotHelper::screenshot(ShotType shotType, bool isHotKey)
{
    ...
    //直接定义矩形选择，忽略扩展
    ShotTypeItem snapType;
    snapType.shotType=FreeShot;
    snapType.autoSavePath="D:/";
    snapType.isManualSave=false;
    snapType.isAutoSave=false;

    if (!isHotKey || !hotKeyNoWait) {
...
QPixmap ScreenShotHelper::getWindowPixmap(HWND winId, ShotType shotType, bool includeCursor, int x, int y, int w , int h) {
    if (shotType == CursorShot) {
        ...
        return ScreenShotHelper::grabWindow(1, winId, shotType, includeCursor, x, y, w, h);
    }
}
...
QString ScreenShotHelper::getPictureName(ShotType shotType)
{
    ...
    QString fileNameTemplate = fileNameTemplate;
    ...
    QString result = transformTemplateStr(fileNameTemplate, kv);
    if (result.isEmpty()) result = transformTemplateStr(defaultTemplate, kv);
    return result;
}
...
bool ScreenShotHelper::savePicture(QWidget *msgBoxParent, QString filePath, QPixmap pixmap) {
    ...
    saved = pixmap.save(filePath, nullptr, -1);
    if (!saved) {
...
QString ScreenShotHelper::transformTemplateStr(QString templateStr, QHash<QString, QString> kv)
{
    QString finalStr;
    QString key;
    bool left = false;
